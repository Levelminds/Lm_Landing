...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $author = trim($_POST['author'] ?? $post['author']);
        $summary = trim($_POST['summary'] ?? $post['summary']);
        $content = trim($_POST['content'] ?? $post['content']);
        $mediaType = $_POST['media_type'] ?? $post['media_type'];
        $mediaUrlInput = trim($_POST['media_url'] ?? $post['media_url']);
        ...
        $processedMediaUrl = $mediaUrlInput !== '' ? $mediaUrlInput : $post['media_url'];
        $imageError = null;
...
<body>
    ...
    <div class="admin-card">
      ...
      <form method="POST" class="row g-4" enctype="multipart/form-data">
        ...
        <div class="col-12">
          ...
          <small class="text-muted">Provide a cover image URL for photo blogs or an embeddable video URL (e.g. YouTube embed). Uploading &amp; cropping a new image below will override this field.</small>
        </div>
        <div class="col-12">
          <label class="form-label fw-semibold">Upload new image (optional)</label>
          <input type="file" name="media_file" class="form-control" accept="image/*" id="editPostImageInput">
          <div class="image-cropper mt-3">
            <div class="ratio ratio-16x9 overflow-hidden position-relative">
              <img src="" alt="Crop selection" id="editPostCropperImage" class="w-100 h-100 d-none" style="object-fit: cover;">
              <div class="w-100 h-100 d-flex align-items-center justify-content-center text-muted" id="editPostCropperPlaceholder">Select an image to start cropping</div>
            </div>
            <div class="image-cropper-controls">
              <button type="button" class="btn btn-outline-secondary btn-sm" id="editPostCropReset" disabled>Reset</button>
              <div class="ms-auto d-flex gap-2">
                <button type="button" class="btn btn-outline-primary btn-sm" id="editPostCropRotate" disabled><i class="bi bi-arrow-repeat me-1"></i>Rotate</button>
                <button type="button" class="btn btn-primary btn-sm" id="editPostCropApply" disabled>Save cropped image</button>
              </div>
            </div>
          </div>
          <div class="mt-3 crop-preview-wrapper d-none" id="editPostCroppedPreviewWrapper">
            <p class="text-muted small mb-2">Cropped image preview</p>
            <img src="" alt="Cropped preview" id="editPostCroppedPreview" class="img-fluid rounded shadow-sm">
          </div>
          <input type="hidden" name="cropped_image" id="editPostCroppedImageInput">
          <?php if ($post['media_type'] === 'photo' && $post['media_url']): ?>
          <div class="mt-3">
            <p class="text-muted small mb-2">Current image</p>
            <img src="<?php echo htmlspecialchars($post['media_url']); ?>" alt="Current cover" style="max-height: 160px; border-radius: 12px; box-shadow: 0 10px 30px rgba(15, 46, 91, 0.12);">
          </div>
          <?php endif; ?>
        </div>
        <div class="col-12">
          <label class="form-label fw-semibold">Short Summary *</label>
          <textarea name="summary" class="form-control tinymce-editor" id="editPostSummary" rows="3" required><?php echo htmlspecialchars($post['summary']); ?></textarea>
        </div>
        <div class="col-12">
          <label class="form-label fw-semibold">Main Content *</label>
          <textarea name="content" class="form-control tinymce-editor" id="editPostContent" rows="10" required><?php echo htmlspecialchars($post['content']); ?></textarea>
        </div>
        <div class="col-md-4">
  ...
  <script>
    ...
    (function initCropper(config) {
      ...
      fileInputId: 'editPostImageInput',
      cropperImageId: 'editPostCropperImage',
      placeholderId: 'editPostCropperPlaceholder',
      applyButtonId: 'editPostCropApply',
      resetButtonId: 'editPostCropReset',
      rotateButtonId: 'editPostCropRotate',
      previewWrapperId: 'editPostCroppedPreviewWrapper',
      previewImageId: 'editPostCroppedPreview',
      hiddenInputId: 'editPostCroppedImageInput'
    });
  </script>
</body>
</html>
