      ...
      function ensureVisitorToken() {
        if (storageAvailable) {
          try {
            ...
            console.warn('Falling back to in-memory visitor token.', error);
          }
        }
      ...
      function getLikedPosts() {
        if (storageAvailable) {
          try {
            ...
            if (!stored) {
              memoryLikedPosts = new Set();
              return new Set(memoryLikedPosts);
            }
            const parsed = JSON.parse(stored);
            if (Array.isArray(parsed)) {
              memoryLikedPosts = new Set(parsed.map(Number));
              return new Set(memoryLikedPosts);
            }
          } catch (error) {
            console.warn('Unable to read liked posts from storage; reverting to in-memory values.', error);
          }
        }
