...
[$featured, $categoryBuckets] = lm_partition_blog_posts($posts);
      ...
      <div class="container">
        <?php if ($featured): ?>
          <?php
            ...
            $featuredTitle = lm_decode_blog_text($featured['title'] ?? '');
            $featuredAuthor = lm_decode_blog_text($featured['author'] ?? 'LevelMinds Team');
            $featuredDate = formatBlogDate($featured['created_at'] ?? '');
            $featuredViews = (int) ($featured['views'] ?? 0);
            $featuredLikes = (int) ($featured['likes'] ?? 0);
            $featuredSummary = lm_blog_excerpt($featured, 220);
            $featuredReadTime = lm_blog_read_time($featured);
            $featuredCategory = lm_normalise_category($featured['category'] ?? 'general');
            $featuredCategoryLabel = lm_blog_category_label($featuredCategory);
            $featuredShareUrl = lm_blog_share_url($featuredId, $baseShareUrl);
          ?>
          <div class="blog-hero__card">
            <div class="row g-0 align-items-stretch">
              <div class="col-lg-6">
                <div class="blog-hero__image h-100">
                  <img src="<?php echo htmlspecialchars(lm_blog_media_url($featured), ENT_QUOTES, 'UTF-8'); ?>" alt="<?php echo htmlspecialchars($featuredTitle, ENT_QUOTES, 'UTF-8'); ?>" class="w-100 h-100">
                </div>
              </div>
    ...
    <section class="blog-categories pb-5">
      <div class="container">
        ...
        <?php foreach ($categoryBuckets as $categoryKey => $categoryPosts): ?>
          <?php
            $categoryTitle = lm_blog_category_label($categoryKey);
          ?>
              ...
              <div class="row g-4">
                <?php foreach ($categoryPosts as $post): ?>
                  <?php
                    ...
                    $postTitle = lm_decode_blog_text($post['title'] ?? '');
                    $postAuthor = lm_decode_blog_text($post['author'] ?? 'LevelMinds Team');
                    $postDate = formatBlogDate($post['created_at'] ?? '');
                    $postSummary = lm_blog_excerpt($post, 160);
                    $postReadTime = lm_blog_read_time($post);
                    $postViews = (int) ($post['views'] ?? 0);
                    $postLikes = (int) ($post['likes'] ?? 0);
                    $shareUrl = lm_blog_share_url($postId, $baseShareUrl);
                  ?>
                  <div class="col-sm-6 col-lg-4 col-xl-3">
                    <article class="blog-card h-100 d-flex flex-column">
                      <div class="blog-card__media">
                        <img src="<?php echo htmlspecialchars(lm_blog_media_url($post), ENT_QUOTES, 'UTF-8'); ?>" alt="<?php echo htmlspecialchars($postTitle, ENT_QUOTES, 'UTF-8'); ?>">
                      </div>
    ...
    (function () {
      ...
      if (modalElement) {
        modalElement.addEventListener('hidden.bs.modal', () => {
          window.scrollTo({ top: lastScrollY || 0, left: 0, behavior: 'auto' });
        });
      }
      ...
      const pendingViewPosts = new Set();
      let lastScrollY = 0;
      ...
      function setModalLoading() {
        ...
        if (modalBody) {
          ...
          wrapper.className = 'py-5 text-center text-muted d-flex flex-column align-items-center gap-3';
          wrapper.innerHTML = '<div class="spinner-border text-primary" role="status" aria-hidden="true"></div><p class="mb-0">Fetching the full story…</p>';
      ...
      async function openModalForPost(trigger) {
        ...
        try {
          ...
          if (!hasViewedPost(postId) && !pendingViewPosts.has(postId)) {
            pendingViewPosts.add(postId);
            const newViews = await incrementViews(postId);
            if (typeof newViews === 'number') {
              markPostViewed(postId);
            }
            pendingViewPosts.delete(postId);
          }
        } catch (error) {
      ...
      document.addEventListener('DOMContentLoaded', () => {
        ...
        const requestedPost = Number(params.get('post') || 0);
        if (requestedPost) {
          const trigger = document.querySelector(`.js-read-more[data-post-id="${requestedPost}"]`);
          if (trigger) {
            openModalForPost(trigger);
          }
